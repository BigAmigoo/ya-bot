...
try:
  ...
  data = response.json()
  chart_data = data['chartPositions']

  # Создаем список словарей для pandas DataFrame
  chart_list = []
  for track in chart_data:
    position = track['chartPosition']['position']
    title = track['track']['title']
    artists = ', '.join([artist['name'] for artist in track['track']['artists']])
    chart_list.append({'Мечто': position, 'Название трека': title, 'Артист': artists})


  df = pd.DataFrame(chart_list)
  df.to_excel('TOP100_yandex_music_chart.xlsx', index=False)
  print("Данные успешно сохранены в TOP100_yandex_music_chart.xlsx")

except requests.exceptions.RequestException as e:
  print(f"Ошибка при выполнении запроса: {e}")
except KeyError as e:
  print(f"Ошибка при разборе JSON: {e}. Возможно, структура JSON изменилась.")
